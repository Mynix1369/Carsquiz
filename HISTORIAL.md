...
### 22.12 Clasificación general por modo, normalizada por dificultad
...
**Solución**: nueva pestaña "General" en `leaderboard.js` (ahora la primera y la que se abre por defecto), junto a las ya existentes Fácil/Medio/Difícil. En la vista General, la consulta a `scores` ya no filtra por dificultad — trae todas las filas de hoy para ese modo, convierte cada puntuación a "% de la puntuación máxima posible en su propia dificultad" (`maxScoreForModeDifficulty()`: 100/200/300 según dificultad en Identificar, fijo en Logos porque ahí la dificultad no cambia el máximo, y n/a en Sonido que no tiene dificultad) y, si un jugador jugó varias dificultades el mismo día, se queda con su mejor porcentaje. Se ordena por ese porcentaje y cada fila muestra una etiqueta con la dificultad que le dio su mejor resultado. Las pestañas Fácil/Medio/Difícil se dejan tal cual (puntos en bruto, ya son justas dentro de una misma dificultad).

Verificado con datos simulados (sustituyendo `sb.from` en consola, sin tocar la base de datos real) en dos partes: la lógica de agrupar/normalizar/quedarse con el mejor porcentaje por jugador (un jugador con una puntuación floja en difícil y una buena en medio debía aparecer con la de medio, y el orden final debía ser por porcentaje y no por puntos en bruto — se comprobó con cinco jugadores de ejemplo y coincidió exactamente), y una captura de pantalla de la pestaña General mostrando el podio con las etiquetas de dificultad.
